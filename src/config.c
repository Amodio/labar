...
find_best_icon(const char *icon_name)
{
	...
	if (verbose >= 2)
		printf("[DBG²] Found %d candidate icon(s) for '%s'\n",
			candidate_count, icon_name);

	// Sort candidates from best to worst quality
	qsort(candidates, candidate_count, sizeof(IconCandidate),
		compare_icons);
	...
	if (verbose >= 2) {
		...
		for (int i = 0; i < candidate_count; i++) {
			const char *type =
				(candidates[i].size == 999) ? "SVG" : "PNG";
			printf("[DBG²]   [%d] %s (%s, %dpx)\n", i,
				candidates[i].path, type, candidates[i].size);
		}
	}
...
list_all_applications(int *count_out)
{
	...
	while ((entry = readdir(dir))) {
		...
		if (count >= capacity) {
			capacity *= 2;
			DesktopEntry **tmp = realloc(entries,
	...
	if (verbose >= 2) {
		...
		for (int i = 0; i < cfg.count; i++) {
			printf("[DBG²]   App[%d]: %s -> %s\n", i,
				cfg.apps[i]->name, cfg.apps[i]->exec);
		}
	}
